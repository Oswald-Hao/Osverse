...
- 启动所有通过重新扫描验证的安装；多个安装位置会提供独立控制，不执行界面提交的任意路径；
- 移除前生成短期、单次有效的预览计划；用户文件移入系统回收站，配置、凭据和会话默认保留；
...
- 加密保存和编辑第三方 API 档案，探测协议兼容性并在确认后更新 CLI 配置；
- 保留最多 200 条脱敏本地操作记录。
...
- 新增所有已验证 CLI、桌面应用和管理工具的启动操作；
- 新增按安装位置预览并安全移除，包含冲突回滚和系统软件包固定提权边界；
- 新增已有 API 档案编辑，密钥始终不回显，更新时要求重新输入；
- 修正 OpenCode 的 OpenAI 兼容配置与 Base URL 规范化；
- CI 改为每个 PR 运行一次，并强制 `功能分支 → dev → beta → main` 推广链路。
