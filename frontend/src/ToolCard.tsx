import type { Component } from './domain'
import StatusBadge from './StatusBadge'
import ToolIcon from './ToolIcon'

const actionLabels: Record<Component['status'], string> = {
  detecting: '配置',
  missing: '安装',
  installed: '配置',
  update_available: '更新',
  conflict: '配置',
  unsupported: '安装',
  broken: '配置',
  installing: '安装',
  failed: '配置',
}

const installableComponents = new Set([
  'claude-code', 'codex-cli', 'opencode-cli', 'deepseek-harness', 'qwen-code', 'kimi-code', 'github-copilot-cli', 'gemini-cli',
  'claude-desktop', 'chatgpt-desktop', 'codex-desktop',
  'opencode-desktop', 'cc-switch', 'cockpit-tools',
])

function ToolCard({ component, onInstall, onLaunch, onRemove }: {
  component: Component
  onInstall?: (id: string) => void
  onLaunch?: (id: string, installationPath: string) => void
  onRemove?: (id: string) => void
}) {
  const action = actionLabels[component.status]
  const canInstall = installableComponents.has(component.id) &&
    ['missing', 'broken', 'failed', 'update_available'].includes(component.status)
  const launchable = ['installed', 'update_available', 'conflict'].includes(component.status)
  const canLaunch = launchable &&
    component.installations.length === 1
  const brokenCLIRecovery = component.category === 'Core CLI' && ['broken', 'failed'].includes(component.status)
  const canRemove = (['installed', 'update_available', 'conflict'].includes(component.status) || brokenCLIRecovery) &&
    (component.installations.length > 0 || component.category === 'Desktop Applications' || brokenCLIRecovery)

  return (
    <li>
      <article className="tool-card">
        <div className="tool-card__topline">
          <div className="tool-card__identity">
            <ToolIcon id={component.id} />
            <div>
              <p className="tool-card__id">{component.id}</p>
              <h4>{component.name}</h4>
            </div>
          </div>
          <StatusBadge status={component.status} />
        </div>

        <p className="tool-card__message">{component.message || '暂无附加信息'}</p>

        {component.installations.length > 0 && (
          <ul className="installations" aria-label={`${component.name} 安装位置`}>
            {component.installations.map((installation, index) => (
              <li key={`${installation.path}-${installation.resolvedPath}-${index}`}>
                <dl>
                  <div>
                    <dt>安装路径</dt>
                    <dd>
                      <code>{installation.path}</code>
                    </dd>
                  </div>
                  <div>
                    <dt>解析目标</dt>
                    <dd>
                      <code>{installation.resolvedPath || installation.path}</code>
                    </dd>
                  </div>
                  {installation.version && (
                    <div>
                      <dt>版本</dt>
                      <dd>{installation.version}</dd>
                    </div>
                  )}
                </dl>
                {launchable && component.installations.length > 1 && (
                  <button type="button" onClick={() => onLaunch?.(component.id, installation.path)}>启动此位置</button>
                )}
              </li>
            ))}
          </ul>
        )}

        <div className="tool-card__footer">
          <span>最低要求：{component.minimumOS}</span>
          <div className="tool-card__actions">
            {canInstall && <button type="button" onClick={() => onInstall?.(component.id)}>{action}<span>官方校验安装</span></button>}
            {canLaunch && <button type="button" onClick={() => onLaunch?.(component.id, component.installations[0].path)}>启动<span>{component.category === 'Core CLI' ? '在终端中启动' : component.installations[0].managed ? '校验后启动' : '启动已检测应用'}</span></button>}
            {canRemove && <button className="tool-card__remove" type="button" onClick={() => onRemove?.(component.id)}>移除<span>先预览再确认</span></button>}
			{!canInstall && !canLaunch && !canRemove && <button type="button" disabled>{action}<span>{component.status === 'installed' ? '未发现可启动入口' : '暂不可用'}</span></button>}
          </div>
        </div>
      </article>
    </li>
  )
}

export default ToolCard
