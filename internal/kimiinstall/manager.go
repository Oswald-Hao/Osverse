...
func (manager *Manager) CreatePlan(ctx context.Context, id string) (install.Plan, error) {
	...
	plan := install.Plan{
		ID: planID, ComponentID: componentID, Name: kimiName, Command: "kimi", Version: kimiVersion,
		DownloadBytes: item.Size, CreatedAt: created, ExpiresAt: created.Add(planLifetime),
		Changes: []install.PlannedChange{
			{Kind: "download", Path: "github.com/MoonshotAI/kimi-code", Description: "下载并校验官方 Kimi Code 独立运行时"},
			{Kind: "directory", Path: paths.finalRoot, Description: "写入 Osverse 管理的不可变 Kimi Code 版本"},
			{Kind: "command", Path: paths.shimPath, Description: "创建 kimi 命令入口"},
		},
	}
	manager.mu.Lock()
...
func (manager *Manager) Start(ctx context.Context, planID string, protocol proxyservice.Protocol, port int) (install.Task, error) {
	...
	task := install.Task{ID: taskID, PlanID: planID, ComponentID: componentID, Phase: "queued", Message: "等待安装 Kimi Code", StartedAt: manager.now().UTC()}
...
func (manager *Manager) runTask(ctx context.Context, taskID string, stored storedPlan, protocol proxyservice.Protocol, port int) {
	...
	switch {
	case err == nil:
		state.public.Phase, state.public.Progress, state.public.Message = "completed", 100, "Kimi Code 安装完成"
