...
func managedPathsFor(home, goos, version string) managedPaths {
	...
	return managedPaths{
		...
		shimPath:    filepath.Join(home, ".local", "bin", "kimi"),
		wrapperPath: filepath.Join(finalRoot, "bin", "kimi"),
		binaryPath:  filepath.Join(finalRoot, "bin", "kimi.real"),
	}
}
...
func inspectOwnedSymlink(path, root string) (symlinkState, error) {
	...
	if err != nil || info.Mode()&os.ModeSymlink == 0 {
		return symlinkState{}, errors.New("managed Kimi entry is not an owned symlink")
	}
	...
	if !pathWithin(root, filepath.Clean(resolved)) {
		return symlinkState{}, errors.New("managed Kimi symlink escapes tool root")
	}
...
func atomicSymlink(destination, target string) error {
	directory, err := os.MkdirTemp(filepath.Dir(destination), ".osverse-kimi-link-")
	if err != nil {
		return err
	}
...
func confirmProfileState(expected profileState) error {
	...
	if !expected.exists {
		...
		return errors.New("shell profile changed during Kimi installation")
	}
