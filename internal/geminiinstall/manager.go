...
func NewManager(home string) (*Manager, error) {
	...
	if _, _, err := artifactsForTarget(runtime.GOOS, runtime.GOARCH); err != nil {
...
func volumeRoot(value string) string {
	return filepath.Clean(filepath.VolumeName(value) + string(filepath.Separator))
}
...
func (manager *Manager) CreatePlan(ctx context.Context, id string) (install.Plan, error) {
	...
	runtimeItem, pack, err := artifactsForTarget(manager.goos, manager.goarch)
	if err != nil {
	...
	if err != nil || planID == "" {
		return install.Plan{}, errors.New("create Gemini CLI plan identifier")
	}
	created := manager.now().UTC()
	plan := install.Plan{
		ID: planID, ComponentID: componentID, Name: geminiName, Command: commandName, Version: geminiVersion,
		DownloadBytes: runtimeItem.Size + pack.Size, CreatedAt: created, ExpiresAt: created.Add(planLifetime),
		Changes: []install.PlannedChange{
			{Kind: "download", Path: "nodejs.org + registry.npmjs.org", Description: "下载并校验官方 Node.js 与 Gemini CLI 固定版本"},
			{Kind: "directory", Path: paths.finalRoot, Description: "写入 Osverse 管理的不可变 Gemini CLI 版本"},
			{Kind: "command", Path: paths.shimPath, Description: "创建 gemini 命令入口"},
		},
	}
	manager.mu.Lock()
	...
	manager.plans[planID] = &storedPlan{public: clonePlan(plan), runtime: runtimeItem, pack: pack}
...
func (manager *Manager) Start(ctx context.Context, planID string, protocol proxyservice.Protocol, port int) (install.Task, error) {
	...
	stored.used = true
	copy := storedPlan{public: clonePlan(stored.public), runtime: stored.runtime, pack: stored.pack, used: true}
	taskID, err := manager.randomID()
	if err != nil || taskID == "" {
		stored.used = false
		manager.mu.Unlock()
		return install.Task{}, errors.New("create Gemini CLI task identifier")
	}
	if ctx == nil {
		ctx = context.Background()
	}
	taskContext, cancel := context.WithCancel(ctx)
	task := install.Task{ID: taskID, PlanID: planID, ComponentID: componentID, Phase: "queued", Message: "等待安装 Gemini CLI", StartedAt: manager.now().UTC()}
...
func (manager *Manager) runTask(ctx context.Context, taskID string, stored storedPlan, protocol proxyservice.Protocol, port int) {
	err := manager.executeFn(ctx, stored, protocol, port, func(phase string, progress int, message string) {
		...
		if state == nil || install.IsTerminalTaskPhase(state.public.Phase) {
			return
		}
	...
	switch {
	case err == nil:
		state.public.Phase, state.public.Progress, state.public.Message = "completed", 100, "Gemini CLI 安装完成"
...
func (manager *Manager) Cancel(id string) error {
	...
	cancel := state.cancel
	terminal := install.IsTerminalTaskPhase(state.public.Phase)
	manager.mu.Unlock()
