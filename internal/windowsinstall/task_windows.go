...
func (manager *Manager) Start(ctx context.Context, planID string, protocol proxyservice.Protocol, port int) (install.Task, error) {
	...
	if err != nil || id == "" {
		return install.Task{}, errors.New("create Windows install task identifier")
	}
	if ctx == nil {
	...
	task := install.Task{
		ID: id, PlanID: stored.public.ID, ComponentID: stored.public.ComponentID,
		Phase: "queued", Progress: 0, Message: "等待安装", StartedAt: manager.now().UTC(),
	}
	manager.mu.Lock()
...
func (manager *Manager) runTask(ctx context.Context, id string, stored storedPlan, protocol proxyservice.Protocol, port int) {
	err := manager.execute(ctx, stored, protocol, port, func(update progressUpdate) { manager.updateTask(id, update) })
	manager.mu.Lock()
	defer manager.mu.Unlock()
	state := manager.tasks[id]
	if state == nil {
	...
	switch {
	case err == nil:
		state.public.Phase, state.public.Progress, state.public.Message = "completed", 100, "安装完成，请在新终端中使用"
		state.public.ErrorCode = ""
	case errors.Is(err, context.Canceled):
		state.public.Phase, state.public.Message, state.public.ErrorCode = "canceled", "安装已取消，原命令未改变", "INSTALL_CANCELED"
	default:
