...
func managedPathsFor(home, goos, version string) managedPaths {
	...
	return managedPaths{
		...
		shimPath:    filepath.Join(home, ".local", "bin", "copilot"),
		wrapperPath: filepath.Join(finalRoot, "bin", "copilot"),
		binaryPath:  filepath.Join(finalRoot, "bin", "copilot.real"),
	}
}
...
func inspectOwnedSymlink(path, root string) (symlinkState, error) {
	...
	if err != nil || info.Mode()&os.ModeSymlink == 0 {
		return symlinkState{}, errors.New("managed Copilot entry is not an owned symlink")
	}
	...
	if !pathWithin(root, filepath.Clean(resolved)) {
		return symlinkState{}, errors.New("managed Copilot symlink escapes tool root")
	}
	return symlinkState{exists: true, target: target}, nil
}

func activateCommand(home string, paths managedPaths) (returnErr error) {
	...
	if err := atomicSymlink(paths.shimPath, paths.wrapperPath); err != nil {
		return err
	}
...
func atomicSymlink(destination, target string) error {
	directory, err := os.MkdirTemp(filepath.Dir(destination), ".osverse-copilot-link-")
	if err != nil {
		return err
	}
...
func confirmProfileState(expected profileState) error {
	...
	if !expected.exists {
		...
		return errors.New("shell profile changed during Copilot installation")
	}
