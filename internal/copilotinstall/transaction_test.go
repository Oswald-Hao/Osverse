...
func TestCommitPayloadRejectsDamagedExistingRuntime(t *testing.T) {
	...
	writeCopilotFixture(t, payload, marker)
	writeCopilotFixture(t, installed, marker)
	if err := os.WriteFile(filepath.Join(installed, "bin", "copilot.real"), []byte("damaged"), 0o700); err != nil {
		t.Fatal(err)
	}
...
func TestNewCommittedPayloadCanBeRolledBackWithoutRemovingExistingRuntime(t *testing.T) {
	...
	writeCopilotFixture(t, newPayload, marker)
	...
	writeCopilotFixture(t, existingPayload, marker)
	writeCopilotFixture(t, existingInstalled, marker)
...
func TestVerifyCopilotRejectsUnexpectedVersionOutput(t *testing.T) {
	binary := filepath.Join(t.TempDir(), "copilot")
	content := "#!/bin/sh\nprintf '%s\\n' 'GitHub Copilot CLI " + copilotVersion + ".' 'unexpected line'\n"
	if err := os.WriteFile(binary, []byte(content), 0o700); err != nil {
		t.Fatal(err)
	}
	if err := verifyCopilot(context.Background(), binary); !errors.Is(err, errVersion) {
		t.Fatalf("verifyCopilot() error = %v", err)
	}
}
...
func TestUnixActivationCreatesOwnedCommandAndProfile(t *testing.T) {
	...
	home := t.TempDir()
	paths := managedPathsFor(home, "linux", copilotVersion)
...
func writeCopilotFixture(t *testing.T, root string, marker []byte) {
	t.Helper()
	for _, relative := range append([]string{".osverse-copilot-runtime"}, criticalPaths("linux")...) {
		path := filepath.Join(root, relative)
		if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
			t.Fatal(err)
		}
		content := []byte(relative)
		if relative == ".osverse-copilot-runtime" {
			content = marker
		}
