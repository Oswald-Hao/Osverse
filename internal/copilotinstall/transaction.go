...
var (
	errDownload        = errors.New("GitHub Copilot CLI download failed")
	errHashMismatch    = errors.New("GitHub Copilot CLI artifact hash mismatch")
	errVersion         = errors.New("GitHub Copilot CLI version verification failed")
	errExternalCommand = errors.New("copilot command is owned by another program")
)
...
func (manager *Manager) execute(ctx context.Context, stored storedPlan, protocol proxyservice.Protocol, port int, progress func(string, int, string)) error {
	...
	archive := filepath.Join(staging, "copilot."+stored.artifact.Format)
	payload := filepath.Join(staging, "payload")
	client, err := manager.client(protocol, port)
	if err != nil || client == nil {
		return errDownload
	}
	progress("downloading", 4, "正在下载 GitHub Copilot CLI 独立运行时")
	if err := downloadArtifact(ctx, client, stored.artifact, archive, func(done int64) {
		progress("downloading", 4+int(done*68/stored.artifact.Size), "正在下载并校验 GitHub Copilot CLI")
	}); err != nil {
		return err
	}
	...
	binary := filepath.Join(payload, "bin", "copilot.real")
	if manager.goos == "windows" {
		binary += ".exe"
	}
	progress("verifying", 74, "正在安全解压 GitHub Copilot CLI")
	if err := extractArtifact(ctx, stored.artifact, archive, binary); err != nil {
		return err
	}
	progress("verifying", 88, "正在验证 GitHub Copilot CLI 版本")
	if err := verifyCopilot(ctx, binary); err != nil {
		return err
	}
	if err := writeWrapper(payload, paths.finalRoot, manager.goos); err != nil {
		return err
	}
	if err := os.WriteFile(filepath.Join(payload, "LICENSE.md"), copilotLicense, 0o600); err != nil {
		return err
	}
	marker := []byte(fmt.Sprintf("component=%s\nversion=%s\ntarget=%s/%s\nsha256=%s\n", componentID, copilotVersion, manager.goos, manager.goarch, stored.artifact.SHA256))
	if err := os.WriteFile(filepath.Join(payload, ".osverse-copilot-runtime"), marker, 0o600); err != nil {
		return err
	}
	progress("committing", 94, "正在原子切换 copilot 命令入口")
	...
	progress("committing", 99, "copilot 命令入口已更新")
	return nil
}
...
func downloadArtifact(ctx context.Context, client *http.Client, item artifact, destination string, progress func(int64)) error {
	...
	request.Header.Set("User-Agent", "Osverse-Copilot-Installer")
	copyClient := *client
...
func extractArtifact(ctx context.Context, item artifact, archive, binary string) error {
	...
	if item.Format == "zip" {
		...
		return extractCopilotZip(ctx, file, info.Size(), binary)
	}
	return extractCopilotTar(ctx, file, binary)
}

func verifyCopilot(ctx context.Context, binary string) error {
	...
	defer cancel()
	command := exec.CommandContext(verifyContext, binary, "--no-auto-update", "--version")
	command.Env = append(os.Environ(), "NO_COLOR=1", "CI=1")
	output, err := command.CombinedOutput()
	lines := strings.Split(strings.TrimSpace(string(output)), "\n")
	if err != nil || len(output) > 4096 || len(lines) < 1 || len(lines) > 2 || strings.TrimSpace(lines[0]) != "GitHub Copilot CLI "+copilotVersion+"." {
		return errVersion
	}
	if len(lines) == 2 && strings.TrimSpace(lines[1]) != "Run 'copilot update' to check for updates." {
		return errVersion
	}
	return nil
}
...
func writeWrapper(payload, finalRoot, goos string) error {
	path := filepath.Join(payload, "bin", "copilot")
	real := filepath.Join(finalRoot, "bin", "copilot.real")
	content := "#!/bin/sh\nset -eu\nexec " + shellQuote(real) + " --no-auto-update \"$@\"\n"
	mode := os.FileMode(0o700)
	if goos == "windows" {
		path += ".cmd"
		content = "@echo off\r\ncall \"%~dp0copilot.real.exe\" --no-auto-update %*\r\nexit /b %ERRORLEVEL%\r\n"
		mode = 0o600
	}
...
func commitPayload(payload, destination string, marker []byte, goos string) (bool, error) {
	...
	existing, err := os.ReadFile(filepath.Join(destination, ".osverse-copilot-runtime"))
...
func criticalPaths(goos string) []string {
	if goos == "windows" {
		return []string{"bin/copilot.real.exe", "bin/copilot.cmd", "LICENSE.md"}
	}
	return []string{"bin/copilot.real", "bin/copilot", "LICENSE.md"}
}
...
func ensureManagedDirectory(home, destination string) error {
	home, destination = filepath.Clean(home), filepath.Clean(destination)
	if !pathWithin(home, destination) && destination != home {
		return errors.New("Copilot managed path escapes user home")
	}
	...
	for _, component := range strings.Split(relative, string(filepath.Separator)) {
		...
		if component == ".." || filepath.Base(component) != component {
			return errors.New("invalid Copilot managed path")
		}
		...
		case !info.IsDir() || info.Mode()&os.ModeSymlink != 0:
			return errors.New("Copilot managed directory is unsafe")
		}
	}
	return nil
}
...
func publicFailure(err error) string {
	...
	case errors.Is(err, errExternalCommand):
		return "copilot 命令入口已被其他程序占用，未修改原安装"
	case errors.Is(err, errHashMismatch):
		return "GitHub Copilot CLI 下载文件校验失败，未安装"
	case errors.Is(err, errUnsafeArchive):
		return "GitHub Copilot CLI 安装包内容不安全，已拒绝安装"
	case errors.Is(err, errVersion):
		return "GitHub Copilot CLI 版本验证失败，未安装"
	case errors.Is(err, errDownload):
		return "GitHub Copilot CLI 下载失败，请检查网络或代理"
	default:
		return "GitHub Copilot CLI 安装失败，原版本未改变"
	}
}
