...
func TestManagerCompletesAndConsumesSingleUsePlan(t *testing.T) {
	...
	if plan.ComponentID != componentID || plan.Command != "qwen" || plan.Version != qwenVersion {
...
func TestManagerCancellationIsTerminalAndReleasesActiveSlot(t *testing.T) {
	now := time.Date(2026, 8, 17, 13, 0, 0, 0, time.UTC)
	...
	waitForQwenPhase(t, manager, task.ID, "canceled")
	...
	waitForQwenPhase(t, manager, second.ID, "completed")
}
...
func waitForQwenPhase(t *testing.T, manager *Manager, taskID, phase string) {
	t.Helper()
