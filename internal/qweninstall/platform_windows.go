...
func managedPathsFor(home, _ string, version string) managedPaths {
	...
	return managedPaths{
		...
		shimPath:    filepath.Join(home, ".local", "bin", "qwen.cmd"),
		wrapperPath: filepath.Join(finalRoot, "bin", "qwen.cmd"),
	}
}

func inspectCommandEntry(paths managedPaths, _ string) error {
	home := filepath.Dir(filepath.Dir(paths.binRoot))
	if err := windowsinstall.ValidateManagedCommandSlot(home, componentID, "qwen"); err != nil {
...
func activateCommand(home string, paths managedPaths, _ string) error {
	if err := windowsinstall.ActivateManagedCommand(home, componentID, "qwen", paths.wrapperPath); err != nil {
		if errors.Is(err, windowsinstall.ErrExternalCommand) {
			return errExternalCommand
		}
		return err
