...
<p align="center">
  ...
  在一个界面里检测、安装、更新、启动和安全移除 Claude Code、Codex CLI、OpenCode、DeepSeek Harness、Qwen Code、桌面应用与第三方 API。
</p>
...
- **已有工具无需搬家。** Claude Code、Codex CLI、OpenCode、Qwen Code 会从用户真实可用的命令路径中发现，不要求放进 Osverse 目录。
...
| --- | --- | --- |
| 核心 CLI | Claude Code、Codex CLI、OpenCode CLI、DeepSeek Harness、Qwen Code 的检测与事务式安装/更新 | 同左；Harness 与 Qwen Code 使用独立的受校验用户级运行时 |
...
- 安装源来自内置白名单，并同时校验文件长度与 SHA-256。Qwen Code 还会拒绝归档路径穿越、链接、设备文件和解压炸弹；DeepSeek Harness 额外使用嵌入式 npm lockfile 对每个包做 SHA-512 校验，且不会执行包内安装脚本。
...
Osverse 是独立项目。Claude、Codex、OpenAI、OpenCode、DeepSeek、Qwen 等名称属于各自权利人；列出它们仅表示兼容，不代表背书或合作关系。
