...
Qwen Code standalone downloads are limited to an exact official release URL, byte length, and SHA-256 digest. GitHub's required single HTTPS handoff is accepted only from the exact `github.com` request to `release-assets.githubusercontent.com`; all other redirect shapes are rejected. Archive extraction is confined to the fixed `qwen-code/` root and rejects traversal, links, special files, hazardous cross-platform names, excess entries, and excess expanded size. Osverse verifies the packaged version with the bundled Node.js runtime before atomically activating its own command wrapper. API profile application preserves unrelated Qwen settings, writes the exact model ID and endpoint, and stores the fallback key only in the current-user `0600` settings file and its protected backup.

Kimi Code standalone downloads are pinned to the exact official release URL, byte length, and SHA-256 digest for each target. Extraction accepts exactly one regular `kimi` or `kimi.exe` entry, rejects links, traversal, extra entries, and oversized output, then requires the binary to report the exact pinned version before activation. The managed wrapper disables Kimi's upstream automatic update so it cannot mutate the immutable Osverse runtime behind the verifier. Profile application owns only a marked `osverse` provider/model block and the selected root model in `~/.kimi-code/config.toml`; it refuses unowned tables, preserves unrelated TOML, writes with current-user-only permissions, and passes the exact vendor model ID without logging the key.

GitHub Copilot CLI standalone downloads use the same fixed-origin, exact-length, SHA-256, single-official-handoff policy. Extraction accepts exactly one regular `copilot` or `copilot.exe` entry and rejects links, traversal, duplicate or extra entries, and oversized output. Osverse executes the extracted binary with auto-update disabled and requires the exact pinned version before activation. The managed wrapper keeps `--no-auto-update` enabled so later upstream downloads cannot bypass this boundary. Authentication tokens remain owned by Copilot CLI and are not read by Osverse.

GitHub-hosted OpenCode Desktop, CC Switch, and Cockpit Tools artifacts share that same one-handoff policy on Linux and Windows. Their fixed catalog URL, exact byte length, and SHA-256 remain the final content identity; Claude Desktop's direct official download does not acquire permission to redirect through this GitHub-only policy.
